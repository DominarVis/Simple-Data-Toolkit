...
/**
  Handles reading a data row from a table.
**/
@:nativeGen
class TableDataRowReaderI extends AbstractTableReader implements TableRowReaderI {
  ...
  private override function elementCheck(oElement : Dynamic) : Bool {
    #if JS_BROWSER
      return (_info.Cell().indexOf(cast(oElement, Element).tagName.toUpperCase()) >= 0);
    #else
      return false;
    #end
  }
