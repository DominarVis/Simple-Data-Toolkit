...
@:expose
@:nativeGen
class ObjectOfObjectsReader<A> extends DataTableReader {
  ...
  **/
  public static function readWholeObject<A>(o : A) : ObjectOfObjectsReader<A> {
    return new ObjectOfObjectsReader(o);
  }
  ...
  public override function nextReuse(rowReader : Null<DataTableRowReader>) : Dynamic {
    ...
    if (rowReader == null) {
      rowReader = ObjectRowReader.readWholeObject(cast _value);
    } else {
      var rr : ObjectRowReader<Dynamic> = cast rowReader;
      ObjectRowReader.readWholeObjectReuse(cast _value, rr);
    }
  ...
  public override function iterator() : Iterator<Dynamic> {
    return new ObjectOfObjectsReader<A>(_o);
  }

  public override function flip() : DataTableWriter {
    return ObjectOfObjectsWriter.writeToWholeObject(_o, null);
  } 
