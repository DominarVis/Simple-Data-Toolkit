...
@:expose
@:nativeGen
class IEEEEventFormat implements CalendarInviteFormat<Reader, Writer> {
    ...
    public function convert(ciInvite : CalendarInvite, wWriter : Writer) : Void {
        ...
        mMap.set("attributes", mAttributes);
        mAttributes.set("created-at", ciInvite.created);
        mAttributes.set("start-time", ciInvite.start);
    ...
    public function read(rReader : Reader) : CalendarInvite {
        ...
        var mAttributes : Map<String, Dynamic>;
        ...
        ciInvite.end = mAttributes.get("end-time");
        ciInvite.summary = mAttributes.get("title");
        ciInvite.uid = mAttributes.get("uid");
        ciInvite.hosts = toArray("name", mAttributes.get("primary-host")).concat(toArray("name", mAttributes.get("cohosts")));
        ciInvite.location = merge(mAttributes, ",", ["address1", "address2", "city", "postal-code", "building", "room-number"]);
        ciInvite.url = mAttributes.get("link");
